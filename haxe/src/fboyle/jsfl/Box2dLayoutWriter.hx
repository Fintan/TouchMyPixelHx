...
//  Box2dLayoutWriter
//
//  Created by Fintan Boyle on 2011-01-21.
//
...
class Box2dLayoutWriter 
{
	...
	private function parseResults(results:Results):String
	{
		...
		for (r in results)
		{
			xml += switch(r.type)
			{
				...
				case "animations":  parseAnimations(r); 
			}
		}
		return xml;
	}
	...
	{
		var xml = '<bitmap ' + parseParameters(result) + ' file="'+result.scope.libraryItem.name.replace("-", "/") + '" linkage="'+result.scope.libraryItem.linkageClassName.replace("-", "/") +'" />\n';
		
		return xml;
	}
	
	public function parseAnimations(result:Result):String {
		var sc = result.scope;
		
		var xml = "<animations";
		
		xml += ' name="'+result.info.name+'"';
		
		var strips:Array<Dynamic> = [];
		...
		if (result.info.isComponent()){
			...
			if (strips != null){
				for (v in strips){
					...
					var ar:Array<Dynamic> = cast v.value.split(":");
					
					if(ar.length < 2){//a second value is for sheetindices when using a spritesheet (not implemented)
						xml += "\t<animation id=\"" +ar[0] + "\" linkage=\""+ ar[0]  + "\"/>\n";
					}else{
						xml += "\t<animation id=\"" +ar[0] +"\" linkage=\""+ ar[0] + "\" sheetindices=\""+ ar[1]  + "\"/>\n";
					}
				}
			}
		}
				...
				for (el2 in el.getTimeline().getChildren())
				{
					...
					if (el2.isComponent())
					{
						switch(el2.libraryItem.linkageClassName)
						{
							...
							case "Def_Animations":
								results.unshift( { type:"animations", info:cast el2, scope:el, children:null } );	
