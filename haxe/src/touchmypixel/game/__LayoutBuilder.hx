...
class LayoutBuilder
{
	...
	private function createGameObject(objectInfo:Fast)
	{
		...
		for (child in objectInfo.x.elements())
		{
			switch(child.nodeName)
			{
				...
				//case "nlastrips": trace("nlastrips"); //handled with body autoSync attribute check
			}
		}
	}
	...
	{ 
		...
		if (gameObject != null)
		{
			...
			<animationedcharacter spritesheet="herosheet.png" linkage="herosheet">
				<nlastrips>
					...
					<strip id="walk" linkage="walk" sheetindices="0123"/>
					<strip id="jump" linkage="jump" sheetindices="456"/>
					<strip id="stand" linkage="stand" sheetindices="7"/>
				</nlastrips>
		...
		{	
			...
			switch(elementInfo.nodeName)
			{
				...
				//nlastrips should ideally be handled here
			}
		}
		...
		if (gameObject != null)
		{
			...
			if(bodyInfo.att.name != null && bodyInfo.att.name != "")
			{
				...
				* Now build the NLAstrips that are auto-synced to the gameObject 
				* (couldn't figure out how to nest <nlastrips> node inside <body> node
				* so I'm using an autosync attribute instead (called autoSyncToNLAs) on the <gameObject> node
				*/
				if( gameObject.info.att.autoSyncToNLAs != "none" ){	
					//trace("autoSyncToNLAs: "+ bodyInfo.att.autoSyncToNLAs);
					createNLAStrips(gameObject.info.att.autoSyncToNLAs, gameObject.info, body);
				}
	...
	*/
	private function createNLAStrips(id:String, gameObject:Fast, ?bod:BuilderBodyObject):Void{
	
		//var dummy:mygame.objects.BigBuck;
