...
import box2D.common.math.B2RayCastInput;
import box2D.common.math.B2RayCastOutput;
...
using box2D.common.math.B2Math;
...
class B2PolygonShape extends B2Shape {
	...
	public override function RayCast( output:B2RayCastOutput, input:B2RayCastInput, xf:B2XForm, childIndex:Int) : Bool
	{
		//B2_NOT_USED(childIndex);

        // Put the ray into the polygon's frame of reference.
        //b2Vec2 p1 = b2MulT(xf.R, input.p1 - xf.position);
		var p1 = B2Math.b2MulMV(xf.R, input.p1.SubtractVV(xf.position));
        //b2Vec2 p2 = b2MulT(xf.R, input.p2 - xf.position);
		var p2 = B2Math.b2MulMV(xf.R, input.p2.SubtractVV(xf.position));
        //b2Vec2 d = p2 - p1;
		var d = p2.SubtractVV(p1);
		...
		if (m_vertexCount == 2)
        {
			...
			var numerator = B2Math.b2Dot(normal, v1.SubtractVV(p1));
			//float32 denominator = b2Dot(normal, d);
			var denominator = B2Math.b2Dot(normal, d);
			...
			var q = p1.AddVV(B2Math.MulFV(t, d));

			// q = v1 + s * r
			// s = dot(q - v1, r) / dot(r, r)
			//--
			var r = v2.SubtractVV(v1);
			var rr = B2Math.b2Dot(r, r);
			if (rr == 0.0)
			{
				return false;
			}
			
			var s = B2Math.b2Dot(q.SubtractVV(v1), r) / rr;
			if (s < 0.0 || 1.0 < s)
			{
				return false;
			}
		...
		else
		{
			...
			for ( i in 0 ... m_vertexCount )
			{
				...
				var numerator = B2Math.b2Dot(m_normals[i], m_vertices[i].SubtractVV(p1));
				//float32 denominator = b2Dot(m_normals[i], d);
				var denominator = B2Math.b2Dot(m_normals[i], d);
			...
			if (index >= 0)
			{
				...
				//output.normal = B2Mat2
				output.normal = B2Math.b2MulMV(xf.R, m_normals[index]);
				return true;
			}
		}
