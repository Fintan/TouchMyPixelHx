...
class B2DistanceJoint extends B2Joint {
	...
	public function new(def:B2DistanceJointDef){
		...
		m_inv_dt = 0.0;
		// Custom code
		m_enforceMinDistance = def.enforceMinDistance;
		m_minLinearCorrection = (def.enforceMinDistance) ? -B2Settings.b2_maxLinearCorrection : 0.0;
		m_longerThanLength = false;
	}
	...
	public override function SolveVelocityConstraints(step:B2TimeStep): Void{
		...
		var dx = b1.m_xf.position.x - b2.m_xf.position.x;
		var dy = b1.m_xf.position.y - b2.m_xf.position.y;
		var d = Math.sqrt(dx * dx + dy * dy);		
		
		m_longerThanLength = d > m_length;
		
		var impulse:Float = 0.0;
		if ( m_enforceMinDistance || m_longerThanLength )
		{
	...
	public override function SolvePositionConstraints():Bool{
		...
		// Custom
		m_longerThanLength = (C >= 0);
		C = B2Math.b2Clamp(C, m_minLinearCorrection, B2Settings.b2_maxLinearCorrection);
	...
	// Custom code for min/max length
	public var m_enforceMinDistance:Bool;
	public var m_minLinearCorrection:Float;
	public var m_longerThanLength:Bool;
}