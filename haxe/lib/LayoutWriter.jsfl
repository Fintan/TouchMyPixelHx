...
touchmypixel.game.LayoutWriter.prototype.parseBitmap = function(result) {
	...
	var $spos = $s.length;
	var xml = ((("<bitmap " + this.parseParameters(result)) + " file=\"") + StringTools.replace(result.scope.libraryItem.name,"-","/") + "\" linkage=\"" + StringTools.replace(result.scope.libraryItem.linkageClassName,"-","/")) + "\" />\n";
	{
		$s.pop();
		return xml;
	}
...
touchmypixel.game.LayoutWriter.prototype.parseBody = function(result) {
	...
	//haxe.Log.trace("this.parseResults(result.children) "+this.parseResults(result.children));
	xml += "</body>\n";
	{
		$s.pop();
		return xml;
...
touchmypixel.game.LayoutWriter.prototype.parseNlaStrips = function(result) {
	$s.push("touchmypixel.game.LayoutWriter::parseNlaStrips");
	
	var $spos = $s.length;
	var xml = ("<nlastrips name=\"nlastrips\" " + this.parseNLAParameters(result)) + ">\n";
	
	var strips = this.getNLAStrips(result);
	for(var i=0; i <strips.length; i++){
		var ar = strips[i].value.split(":");
		// haxe.Log.trace("item 1 - "+ ar[0]);
		xml += "\t<nlastrip id=\"" +ar[0] +"\" linkage=\""+ ar[1] + "\" sheetindices=\""+ ar[2]  + "\"/>\n";
	}
...
touchmypixel.game.LayoutWriter.prototype.parseGameObject = function(result) {
	...
	xml += this.parseResults(result.children);
	haxe.Log.trace("parseGameObject this.parseResults(result.children) "+this.parseResults(result.children));
	xml += "</gameObject>\n";
	{
		$s.pop();
		return xml;
...
touchmypixel.game.LayoutWriter.prototype.parseNLAParameters = function(result) {
...
touchmypixel.game.LayoutWriter.prototype.getNLAStrips = function(result) {
	$s.push("touchmypixel.game.LayoutWriter::getNLAStrips");
	var $spos = $s.length;
	var sc = result.scope;
	var strips = [];
	if(touchmypixel.game.utils.JSFLTools.isComponent(result.info)) {
				...
				var el1 = el;
				{
					...
					while(_g2 < _g3.length) {
						...
						if(touchmypixel.game.utils.JSFLTools.isComponent(el21)) {
							//haxe.Log.trace("el21.libraryItem.linkageClassName "+el21.libraryItem.linkageClassName);
							...
							case "Def_NLAStrips":{
								results.unshift({ type : "nlaStrips", info : el21, scope : el1, children : null});
							}break;
							}
						}
					}
